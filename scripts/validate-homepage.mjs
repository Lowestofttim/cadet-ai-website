...
const voiceSampleVersion = '20260718-premium-voice-speed-5';
...
assert.match(index, /site\.js\?v=20260718-premium-voice-speed-5/, 'Homepage should cache-bust the latest interaction script');
...
for (const voicePage of ['index.html', 'plans.html']) {
  ...
  for (const [, classes, id, src, rate, tier] of voiceCardMatches) {
    ...
    assert.ok(Number(rate) >= 1.58 && Number(rate) <= 1.66, `${voicePage} ${id} should sound launch-brisk`);
